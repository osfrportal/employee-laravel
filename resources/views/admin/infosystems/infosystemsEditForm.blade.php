        ...
        <div class="mb-3 row">
            ...
            <div class="col-sm-10">
                <select class="form-select form-select-sm @error('syncWithIS') is-invalid @enderror" id="syncWithIS"
                    ...
                    <option value="1" @selected(old('syncWithIS', !is_null($infoSystemData) ? (bool) $infoSystemData->isys_data->syncWithIS : ''))==1)>Включено</option>
                    <option value="0" @selected(old('syncWithIS', !is_null($infoSystemData) ? (bool) $infoSystemData->isys_data->syncWithIS : ''))==0)>Не требуется</option>
                </select>
                @error('syncWithIS')
